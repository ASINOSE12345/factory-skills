...
describe("project-coverage — CLI contract (subprocess on dist)", () => {
  ...
  it("JSON output carries run/trace metadata (tool_version / tool_git_sha / generated_at)", () => {
    const root = fullFixture();
    const res = spawnSync("node", [DIST, "--factory-root", root, "--format", "json"], { encoding: "utf8" });
    expect(res.status).toBe(0);
    const parsed = JSON.parse(res.stdout) as Record<string, unknown>;
    expect(parsed.tool).toBe("project-coverage-cli");
    ...
    // metadata must NOT disturb the baseline analysis
    expect((parsed.summary as { repos_total: number }).repos_total).toBeGreaterThan(0);
  });
  ...
  it("Markdown output carries the run/trace metadata footer", () => {
    ...
    const res = spawnSync("node", [DIST, "--factory-root", root, "--format", "md"], { encoding: "utf8" });
    expect(res.status).toBe(0);
    expect(res.stdout).toMatch(/Generated by `project-coverage-cli`/);
  });
